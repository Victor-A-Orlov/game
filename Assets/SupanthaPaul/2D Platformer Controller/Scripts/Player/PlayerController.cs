...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
		...
		private BoxCollider2D ladderTopCollider;
		...
		public void SetCanClimbLadder(bool canClimb, BoxCollider2D topCollider = null)
		{
			this.canClimb = canClimb;
			ladderTopCollider = topCollider;
		}
		...
		private void Update()
		{
		    ...
		    if (canClimb && Input.GetKeyDown(KeyCode.E))
			{
				...
				m_rb.gravityScale = isClimbing ? 0 : 1;
				m_rb.velocity = new Vector2(m_rb.velocity.x, 0f);
				if (ladderTopCollider != null)
				{
					ladderTopCollider.enabled = !isClimbing; // Включаем или отключаем верхний коллайдер лестницы
				}
			}

			// Если персонаж в состоянии взбираться
			if (isClimbing)
			{
				var verticalInput = Input.GetAxis("Vertical");
				m_rb.velocity = new Vector2(m_rb.velocity.x, verticalInput * climbSpeed);
				// Отпустить лестницу при достижении краев
				if(verticalInput > 0 && ladderTopCollider != null && transform.position.y > ladderTopCollider.bounds.max.y)
				{
					isClimbing = false;
					m_rb.gravityScale = 1;
				}
